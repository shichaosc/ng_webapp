...
def get_virtualclass_lesson(vc, user_student_info):
    if vc.class_type_id == 1:  # 1对1
        ...
        if not last_vc:
            lesson = user_student_info.lesson
            vc.first_course = 1
        else:
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--excle_name',
                            dest='excle_name',
                            default=1)
    ...
    def handle(self, *args, **kwargs):
        ...
        if sheet_name:
            ...
            for cases in list(sh.rows)[1:]:
                '''教室ID	学员名称	家长账户名	教室类型	老师账户名	上课时间'''
                tk_class_id = str(cases[0].value).strip()
                student_name = str(cases[1].value).strip()
                parent_user_name = cases[2].value
                class_type = cases[3].value
                tutor_user_name = cases[4].value
                start_time = cases[5].value
                start_time = datetime.datetime.strptime(start_time, '%Y-%m-%d %H:%M:%S').astimezone(tz=pytz.UTC)
                start_time_utc = start_time - datetime.timedelta(hours=8)

                if class_type == '2':  # 一对一
                    continue
                user_tutor_info = TutorInfo.objects.filter(Q(username=tutor_user_name)|Q(email=tutor_user_name)|Q(phone=tutor_user_name)).first()
                if not user_tutor_info:
                    print('not found tutor, tutor_user_name is {}'.format(tutor_user_name))
                    continue
                user_parent_info = UserParentInfo.objects.filter(Q(username=parent_user_name)|Q(email=parent_user_name)|Q(phone=parent_user_name)).first()
                if not user_parent_info:
                    print('not found parent, parent_user_name is {}'.format(parent_user_name))
                    continue
                user_student_info = UserStudentInfo.objects.filter(parent_user_id=user_parent_info.id, real_name=student_name).first()
                if not user_student_info:
                    print('not found student, student_user_name is {}'.format(student_name))
                    continue
                vc = VirtualclassInfo.objects.filter(tutor_user_id=user_tutor_info.id, start_time=start_time_utc).first()
                if vc:
                    if not vc.tk_class_id:
                        vc.tk_class_id = tk_class_id
                    continue
                logininfo_result = getlogininfo(tk_class_id)
                self.create_virtualclass(user_tutor_info, user_student_info, tk_class_id, start_time_utc, logininfo_result)

    def create_virtualclass(self, user_tutor_info, user_student_info, tk_class_id, start_time, logininfo_result):
        ...
        vc.end_time = start_time + datetime.timedelta(minutes=55)
        vc.status = VirtualclassInfo.FINISH_ABNOMAL
        vc.reason = 0
        vc.virtualclass_type_id = 1
        vc = get_virtualclass_lesson(vc, user_student_info)
        ...
        user_student_ids = [user_student_info.id]
